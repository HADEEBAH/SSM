              ...
              <v-tabs-items v-model="student_tab" class="rounded-lg">
                ...
                <v-tab-item valus="student potential">
                  ...
                  <template v-else>
                    ...
                    <template v-else>
                      <div
                        v-for="(coach, coach_index) in coachListPotential(
                          coach_list
                        )"
...
export default {
  ...
  data: () => ({
    ...
    student_index: null,
    student_potent_index: null,
    arr_index: "",
    searchBool: false,
    error_data: false,
    error_data_potent: false,
    students_data: [],
  ...
  methods: {
    ...
    searchStudentPotential(search) {
      if (search !== "") {
        ...
        this.student_potent_index = null; // Reset the index
        this.arr_index = "";
        for (let i = 0; i < this.coach_list.length; i++) {
          for (
            let j = 0;
            j < this.coach_list[i].studentPotentialArr.length;
            j++
          ) {
            const student = this.coach_list[i].studentPotentialArr[j];
            ...
            if (
              ...
              this.student_potent_index = { dataIndex: i, student_index: j };
              ...
              this.GetStudentPotentialByCoach({
                course_id: this.$route.params.course_id,
                coach_id:
                  this.coach_list[this.student_potent_index.dataIndex].coachId,
              });
              this.selected_coach_potential =
                this.student_potent_index.dataIndex;

              this.error_data_potent = false;
              return; // Exit once a match is found
            } else {
              this.selected_coach_potential = null;
              this.error_data_potent = true;
            }
          }
        }
      } else {
            ...
            if (
              ...
              for (const indexDate in this.coach_list[this.arr_index]
                ...
                if (checkName >= 1 || checkLName >= 1 || checkFName >= 1) {
                  ...
                  const matchingDates = uniqueDates.filter((date) =>
                    uniqueDates.includes(date)
                  );

                  const indices = matchingDates.map((date) =>
                    uniqueDates.indexOf(date)
                  );

                  const matchingDatesObj = matchingDates.map(
                    (date) => new Date(date)
                  );
                  // Flatten the studentData array
                  const flattenedData = this.students_data.flat();

                  // Filter records based on matching dates
                  const filteredData = flattenedData.filter((record) => {
                    const recordDate = new Date(record.date);
                    return matchingDatesObj.some(
                      (date) =>
                  ...
                  // ยังไม่เช็คอิน
                  uniqueDatesTwo = [...new Set(checkDateTwo)];

                  const matchingDateNoCheckIn = uniqueDatesTwo.filter((date) =>
                    this.dateList.includes(date)
                  );

                  indicesNoCheckIn = matchingDateNoCheckIn.map((date) =>
                    this.dateList.indexOf(date)
                  );

                  const matchingDatesObjNoCheckIn = matchingDateNoCheckIn.map(
                    (date) => new Date(date)
                  );
                  ...
                  const filteredDataNoCheckIn = flattenedDataNoCheckIn.filter(
                    (record) => {
                      ...
                      return matchingDatesObjNoCheckIn.some(
                        (date) =>
                          date.toISOString().slice(0, 10) ===
                          recordDateTwo.toISOString().slice(0, 10)
                      );
                    }
                  );
