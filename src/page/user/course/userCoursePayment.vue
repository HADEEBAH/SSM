<template>
  <v-app>
    <v-container >
        ...
        <v-row >
            <v-col align="center" class="text-lg font-bold " :class="payload.responseMessage !== 'Data Incomplete' ? 'text-[#58A144]' : 'text-[#e00]'">
                {{ paymentStatus  }}
            </v-col>
        </v-row>
        <div align="center">
            <div style="width: 400px;">
                ...
                <v-row>
                    ...
                    <v-col align="right">{{ formatDate(payload.transactionDate) }}</v-col>
                </v-row>
                <v-row>
                    <v-col  cols="6" sm="4" align="left"> เวลาชำระเงิน :</v-col>
                    <v-col align="right">{{ formatTime(payload.transactionTime)}}</v-col>
                </v-row>
                <v-row>
                    <v-col  cols="6" sm="4" align="left"> ยอดที่ต้องชำระ :</v-col>
                    <v-col align="right">{{parseFloat(payload.receiveAmount).toLocaleString( undefined,{minimumFractionDigits: 2, maximumFractionDigits: 2}) }}บาท</v-col>
                </v-row>
                <v-row>
                ...
                <v-row dense>
                    <v-col cols="12" v-if="payload.responseMessage === 'Data Incomplete' || payload.responseMessage === 'Cancel'">
