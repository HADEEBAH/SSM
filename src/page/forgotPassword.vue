  ...
  <v-container>
    <v-row dense>
      <v-col cols="12" class="text-center m-auto">
        <v-img src="@/assets/changePassword/confirm_current_password.svg" class="max-w-md m-auto"/>
      </v-col>
      <v-col cols="12" class="text-center">
        <label class="font-weight-bold">
          เลือกวิธีการเปลี่ยนรหัสผ่าน
        </label>
      </v-col>
      <v-col cols="12" class="text-center">
        <v-radio-group
  ...
  export default {
    ...
    methods: {
      ...
      async sendTypeChangePassword () {
        this.loading = true
        if (this.type !== "" ) {
          await this.SendTypeForgotPassword({type: this.type,value: this.type === "phone" ? this.value.replaceAll("-", "") : this.value, username: this.username})
          if (this.responseTypeForgotPassword) {
            ...
            let error_message = ""
            if (this.type === "phone") {
              if (this.responseTypeForgotPasswordMessage === "Parameter missing. Required username parameter.") {
                error_message = "กรุณากรอก ชื่อผู้ใช้ (username)"
              } else if (this.responseTypeForgotPasswordMessage === "Parameter missing. Required tel_no parameter.") {
                error_message = "กรุณากรอก เบอร์โทรศัพท์"
              } else if (this.responseTypeForgotPasswordMessage === "username and Tel not found") {
              ...
              if (this.responseTypeForgotPasswordMessage === "Parameter missing. Required username parameter.") {
                error_message = "กรุณากรอก ชื่อผู้ใช้ (username)"
              } else if (this.responseTypeForgotPasswordMessage === "Parameter missing. Required email parameter.") {
                error_message = "กรุณากรอก เบอร์โทรศัพท์"
              } else if (this.responseTypeForgotPasswordMessage === "username and email not found.") {
    ...
    computed: {
      ...mapGetters({
        responseTypeForgotPassword: "ResetPasswordModules/responseTypeForgotPassword",
        responseTypeForgotPasswordMessage: "ResetPasswordModules/responseTypeForgotPasswordMessage",
      }),
