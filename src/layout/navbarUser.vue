...
<template>
  <v-app v-if="!checkShowConsentPage">
    <v-layout>
      <v-app-bar
        app
        clipped-right
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      get_concent_reg: "RegisterModules/getConcentReg",
    }),

    checkShowConsentPage() {
      if (!this.get_concent_reg || !this.get_concent) {
        const roleId = this.user_detail?.role_id;
        return roleId === "R_4" || roleId === "R_5" || roleId === "";
      }
  ...
  methods: {
    ...
    async handlePDPAAccepted() {
      ...
      this.$store.commit("RegisterModules/SetGetConcent", true);
    },
    handlePDPACanceled() {
      this.get_concent = true;
    },
    seeMoreNoti() {
