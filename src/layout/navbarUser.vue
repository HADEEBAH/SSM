<template>
  <div>
    <!-- v-if="
        !concent_detail.concent_data &&
        (user_detail?.role_id === 'R_4' ||
          user_detail?.role_id === 'R_5' ||
          user_detail?.role_id === '')
      " -->
    <concentPage
      v-if="
        user_detail
          ...
          : !concent_detail.concent_data
      "
      @pdpa-accepted="handlePDPAAccepted"
      @pdpa-canceled="handlePDPACanceled"
    />
      ...
      <v-app>
        <v-layout>
          ...
          >
            ...
            <v-list nav>
              <div
                v-for="(list, list_index) in menu_drawer_list"
                :key="list_index"
              >
                <template
            ...
            <v-card-text>
              ...
              >
                ...
                >
                  <v-row
                    dense
                    :class="$vuetify.breakpoint.mdAndUp ? 'pl-5' : ''"
                  >
...
export default {
  ...
  methods: {
    ...
    async handlePDPAAccepted() {
      await this.GetConcent();
      this.concent_detail.concent_data = await this.get_concent;
    },
    handlePDPACanceled() {
      this.concent_detail.concent_data = true;
    },
    seeMoreNoti() {
